...
import { TextField, FormControl, InputLabel, Select, MenuItem, RadioGroup, FormControlLabel, Radio, Checkbox, FormGroup } from '@mui/material';
import type { FieldSchema } from '../types';
import { buildRHFRules } from '../utils/validation';

export default function FieldRenderer({ field, register, errors }: { field: FieldSchema; register: any; errors: any }) {
  const rules = buildRHFRules(field.validations ?? undefined);
  ...
  switch (field.type) {
    ...
    case 'text area':
      return (
        <div style={{ marginTop: 12 }}>
          <TextField label={field.label} multiline rows={4} fullWidth defaultValue={field.defaultValue ?? ''} {...register(field.key, rules)} error={!!errors[field.key]} helperText={errors[field.key]?.message as string} />
        </div>
      );
    case 'select':
      return (
        <div style={{ marginTop: 12 }}>
          <FormControl fullWidth>
            ...
            <Select defaultValue={field.defaultValue ?? ''} label={field.label} {...register(field.key, rules)}>
              {(field.options || []).map(opt => <MenuItem key={opt} value={opt}>{opt}</MenuItem>)}
            </Select>
            <div style={{ color: 'red' }}>{errors[field.key]?.message}</div>
          </FormControl>
        </div>
      );
    case 'radio':
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <RadioGroup defaultValue={field.defaultValue ?? ''}>
            {(field.options || []).map(opt => (
              <FormControlLabel key={opt} value={opt} control={<Radio {...register(field.key, rules)} />} label={opt} />
            ))}
          </RadioGroup>
          <div style={{ color: 'red' }}>{errors[field.key]?.message}</div>
        </div>
    ...
    case 'checkbox':
      return (
        <div style={{ marginTop: 12 }}>
          ...
          <FormGroup>
            {(field.options || []).map(opt => (
              <FormControlLabel key={opt} control={<Checkbox {...register(field.key)} />} label={opt} />
            ))}
          </FormGroup>
        </div>
      );
